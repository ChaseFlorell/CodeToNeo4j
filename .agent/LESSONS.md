...
- **Chain of Responsibility in Option Handlers**: When implementing a chain of handlers for CLI options, ensure that handlers intended to be additive (like `PurgeConfirmationHandler` and `MsBuildRegistrationHandler`) return `true` to allow the chain to continue. Returning `false` terminates the processing. This is useful for exclusive modes, such as a confirmed `--purge-data` command, where the tool should stop after the purge and not proceed to data ingestion.
- **Selective Purging with Skip-Dependencies**: When implementing a partial purge (e.g., using `--skip-dependencies`), ensure that only the `Dependency` nodes are preserved. All other nodes with `CodeToNeo4j: true` (such as `File`, `Symbol`, `Commit`, `Author`, and `Project`) should still be purged to ensure a clean state for the project, while keeping shared dependencies that might be used by other projects. Use the `$purgeDependencies` boolean parameter in Cypher to conditionally skip `MATCH` and `DETACH DELETE` clauses ONLY for the `:Dependency` label.
- **Orphaned Node Cleanup during Purge**: To prevent "leaked" data when a project-specific purge is performed, the `PurgeData.cypher` query should not only delete nodes explicitly linked to the target project but also any "orphaned" nodes (those with `CodeToNeo4j: true` but NO links to ANY `Project` node, or in the case of `Author`, no links to ANY `Commit`). This ensures that if relationships were broken or ingestion was incomplete, the database can still be returned to a clean state. For example, use `($repoKey IS NULL OR EXISTS { (:Project {key: $repoKey})-[:HAS_FILE]->(n) } OR NOT EXISTS { (:Project)-[:HAS_FILE]->(n) })` in the `WHERE` clause.
...
- All nodes created or updated by CodeToNeo4j MUST include the `CodeToNeo4j: true` attribute. This is critical for the `--purge-data` command to correctly identify and delete only the data managed by the tool. 
- Double-check all Upsert Cypher files (`UpsertFile.cypher`, `UpsertCommit.cypher`, `UpsertSymbols.cypher`, `UpsertDependencies.cypher`, `UpsertProject.cypher`) when adding new node types or relationships.
- Author Node Tagging: Specifically, `Author` nodes are merged from multiple sources (file metadata and git commits). Ensure `SET a.CodeToNeo4j = true` is present in ALL `MERGE (a:Author ...)` blocks to prevent orphaned authors from being skipped during purges.
