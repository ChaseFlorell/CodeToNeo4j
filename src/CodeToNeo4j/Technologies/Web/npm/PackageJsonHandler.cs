...
namespace CodeToNeo4j.Technologies.Web.npm;
	...
	: PackageDependencyHandlerBase(fileSystem, textSymbolMapper, configurationService)
{
		...
		Accessibility minAccessibility)
	{
		...
		try
		{
			...
			await ExtractDependencySection(root, "dependencies", fileKey, relativePath, fileNamespace, symbolBuffer, relBuffer, packageDir, urlNodes)
	...
	private async Task CollectNpmUrls(string name, string packageDir, List<UrlNode> urlNodes)
	{
		...
		try
		{
			...
			if (root.TryGetProperty("homepage", out var homepage))
			{
				...
				if (!string.IsNullOrEmpty(url))
				{
					urlNodes.Add(new(depKey, $"url:{url}", url));
				}
			}
			...
			if (root.TryGetProperty("repository", out var repository))
			{
				...
				{
					urlNodes.Add(new(depKey, $"url:{repoUrl}", repoUrl));
				}
			}
		}
		catch (Exception ex)
