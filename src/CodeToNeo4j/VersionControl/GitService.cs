	...
	ILogger<GitService> logger) : IVersionControlService
{
	public async Task LoadMetadata(string workingDirectory, IEnumerable<string> includeExtensions)
	{
		...
		HashSet<string> includedExtensionsSet = includeExtensions.ToHashSet(StringComparer.OrdinalIgnoreCase);
		...
		using var p = Process.Start(psi) ?? throw new("Failed to start git log for metadata pre-fetch.");

		Dictionary<string, List<(string Author, DateTimeOffset Date, string Hash, string? Refs)>> fileHistory = new(StringComparer.OrdinalIgnoreCase);
	...
	public async Task<DiffResult> GetChangedFiles(string diffBase, string workingDirectory, IEnumerable<string> includedExtensions)
	{
		...
		using var p = Process.Start(psi) ?? throw new("Failed to start git process.");
		...
		if (p.ExitCode != 0)
		{
			...
			throw new($"git diff failed: {err}");
		}

		HashSet<string> modifiedSet = new(StringComparer.OrdinalIgnoreCase);
		HashSet<string> deletedSet = new(StringComparer.OrdinalIgnoreCase);

		HashSet<string> includedExtensionsSet = includedExtensions.ToHashSet(StringComparer.OrdinalIgnoreCase);
		...
		return new(modifiedSet, deletedSet, []);
	}
	...
	public async Task<int> GetCommitCount(string range, string workingDirectory)
	{
		...
		using var p = Process.Start(psi) ?? throw new("Failed to start git rev-list.");
		var output = await p.StandardOutput.ReadToEndAsync().ConfigureAwait(false);
		await p.WaitForExitAsync().ConfigureAwait(false);
	...
	public async Task<IEnumerable<CommitMetadata>> GetCommitBatch(string range, string workingDirectory, int batchSize, int skip)
	{
		...
		using var p = Process.Start(psi) ?? throw new("Failed to start git log batch.");
		var output = await p.StandardOutput.ReadToEndAsync().ConfigureAwait(false);
		await p.WaitForExitAsync().ConfigureAwait(false);
	...
	public async Task<FileMetadata> GetFileMetadata(string filePath, string workingDirectory)
	{
		...
		using var p = Process.Start(psi) ?? throw new($"Failed to start git log for {filePath}");
		var output = await p.StandardOutput.ReadToEndAsync().ConfigureAwait(false);
		await p.WaitForExitAsync().ConfigureAwait(false);
	...
	private static async Task<string> GetGitRoot(string workingDirectory)
	{
		...
		using var p = Process.Start(psi) ?? throw new("Failed to start git process to find repo root.");
		var output = await p.StandardOutput.ReadToEndAsync().ConfigureAwait(false);
		await p.WaitForExitAsync().ConfigureAwait(false);

		if (p.ExitCode != 0)
		{
			throw new("Could not find git repository root.");
		}
