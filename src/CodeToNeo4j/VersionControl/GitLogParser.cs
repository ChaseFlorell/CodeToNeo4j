	...
	IFileSystem fileSystem) : IGitLogParser
{
		...
		foreach (var line in lines)
		{
			...
			if (line.StartsWith("COMMIT|"))
			{
				...
				if (headerParts.Length >= 5)
				{
					...
					{
						currentCommit = new(headerParts[0], headerParts[1], headerParts[2], date, headerParts[4], []);
					}
				}
			}
			else
			{
				if (currentCommit != null)
				{
					...
					if (fileParts.Length >= 2)
					{
						...
						changedFiles.Add(new(fullPath, status.StartsWith('D')));
					}
				}
			}
		}
		...
		IList<(string Author, DateTimeOffset Date, string Hash, string? Refs)> history)
	{
		...
		return new(created, lastModified, authors, [.. hashes], [.. tags]);
	}
	...
	public FileMetadata ParseSingleFileLog(string output)
	{
		...
		return history.Count == 0
			? new(DateTimeOffset.MinValue, DateTimeOffset.MinValue, [], [], [])
