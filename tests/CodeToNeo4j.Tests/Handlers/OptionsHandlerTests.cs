...
public class OptionsHandlerTests
{
	...
	public async Task GivenPurgeDataTrue_WhenPurgeExecutionHandlerCalled_ThenPurgeDataIsCalledAndChainStops()
	{
		...
		PurgeExecutionHandler handler = new(graphService);
		var nextHandler = A.Fake<IOptionsHandler>();
		handler.SetNext(nextHandler);
	...
	public async Task GivenPurgeDataTrue_WhenPurgeExecutionHandlerCalled_ThenPurgeDataIsCalledWithCorrectPurgeDependencies(bool skipDependencies,
		bool expectedPurgeDependencies)
	{
		// arrange
		var graphService = A.Fake<CodeToNeo4j.Graph.IGraphService>();
		PurgeExecutionHandler handler = new(graphService);
		var options = CreateOptions(true, skipDependencies);

		// act
		await handler.Handle(options);

		// assert
		A.CallTo(() => graphService.PurgeData(A<string>._, A<IEnumerable<string>>._, A<string>._, expectedPurgeDependencies, A<int>._))
	...
	public async Task GivenPurgeDataTrueAndDefaultExtensions_WhenPurgeExecutionHandlerCalled_ThenPurgeDataIsCalledWithNullExtensions()
	{
		...
		var options = CreateOptions(true, includeExtensions: allSupportedExtensions);
